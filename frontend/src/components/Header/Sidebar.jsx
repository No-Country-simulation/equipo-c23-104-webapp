...
<div className="h-full px-3 pb-4 pt-3 overflow-y-auto bg-white dark:bg-[#4A494A] mt-4 transition-all duration-500">
  <ul className="space-y-0 font-medium">
    {[
      ...
      >
        ...
        >
          <i
            className={`fa-solid ${item.icon} text-[#00bf00] dark:text-white transition-all duration-100 group-hover:text-white`}
          ></i>
          {isTextVisible && (
            <span
  ...
  <button
    ...
    type="button"
    className={`border hover:border-transparent bg-[#00bf00] text-white dark:hover:border-white font-medium rounded transition-all duration-200 mt-3 ${
      isTextVisible
...
<nav className="fixed bottom-0 left-0 w-full bg-white border-t border-[#A19FA1] dark:bg-[#4A494A] dark:border-gray-700 flex justify-around py-2 sm:hidden">
{[
  { icon: "fa-house", label: t("home") },
  { icon: "fa-users", label: t("community") },
  { icon: "fa-bell", label: t("notifications") },
